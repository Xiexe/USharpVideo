...
namespace UdonSharp.Video
{
    ...
    public class USharpVideoPlayer : UdonSharpBehaviour
    {
        ...
        [Tooltip("Defaults the video player to be on.")]
        [PublicAPI]
        public bool defaultPlayerTurnedOn = true;

        [PublicAPI]
        public bool isSubscribedToPlayer = false;
...
#pragma warning restore CS0414
        ...
        void Start()
        {
            ...
            isSubscribedToPlayer = defaultPlayerTurnedOn;
            SetUIPower(defaultPlayerTurnedOn);
        ...
        bool needUpdatePowerOffStatus = true;
        private void Update()
        {
            ...
            {
                if (_ownerPlaying && _syncedURL != null)
                {
                    BlinkUIPower();

                    if (needUpdatePowerOffStatus)
                    {
                        SetStatusText("Press Power to join video in progress!");
                        needUpdatePowerOffStatus = false;
                    }
                }
                else
                {
        ...
        [PublicAPI]
        public void SetSubscribedStatus(bool subscribed)
        {
            isSubscribedToPlayer = subscribed;

            if (isSubscribedToPlayer)
            {
                // Just in-case ownership got transferred to someone not watching.
                PassOwnershipToNewUser();
            ...
            SetUIPower(isSubscribedToPlayer);
            needUpdatePowerOffStatus = true;
        }

        private void PassOwnershipToNewUser()
        {
            //Do this so the video doesn't restart for everyone
            //Kinda a hack, but its the quickest way to fix this bug. (owner turns off player, turns back on player, video restarts)
            if (!Networking.IsOwner(gameObject))
                return;
            ...
            {
                VRCPlayerApi potentialOwner = players[i];
                VRCPlayerApi currentOwner = Networking.GetOwner(gameObject);
                if (potentialOwner != currentOwner)
                {
                    LogMessage($"User {currentOwner} passing ownership to {potentialOwner}");
                    Networking.SetOwner(potentialOwner, gameObject);
                    break;
                }
            }
        }
...
#region UI Control handling
        ...
        void SetUIPower(bool powerOn)
        {
            foreach (VideoControlHandler handler in _registeredControlHandlers)
                handler.SetPower(powerOn);
        }

        void BlinkUIPower()
        {
            foreach (VideoControlHandler handler in _registeredControlHandlers)
                handler.BlinkPowerButton();
        }
